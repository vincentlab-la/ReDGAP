...
author: "Layla Ahmed and Erika Tavares"
date: "2025-12-09"
...
#You are currently using the Inheritance Filtering script; this script is designed to utilize gene names for matching variants. Note that you will need a column called "gene_symbol" for this; if you do not have this column name, edit your data before running this.Use | to separate gene names for variants overlapping multiple genes.
...
sheet_names <- list()
Dnumber <<- as.character(readline(prompt ="Please write the Sample ID:"))
...
find_recessive_candidates <- function(wb) {
  ...
  for (i in 1:length(full_sheet_list)) {
    ...
    if("gene_symbol" %in% colnames(current_sheet)) {
     # Ensure column is character
  current_sheet$gene_symbol <- as.character(current_sheet$gene_symbol)
  
  current_sheet$gene_symbol_split <- strsplit(current_sheet$gene_symbol, "\\|")
  genes_expanded <- unlist(current_sheet$gene_symbol_split)

      gene_counts <- table(genes_expanded)
  ...
  separate_rows(gene_symbol, sep = "\\|") %>%   # <-- split gene symbols
  group_by(gene_symbol) %>%
  summarise(
    GeneScore_AvgAll = mean(VariantScore, na.rm = TRUE),
    GeneScore_AvgTop2 = mean(sort(VariantScore, decreasing = TRUE)[1:min(2, n())], 
                             na.rm = TRUE),
    .groups = "drop"
  )
...
#
  # Drop problematic list-columns (like gene_symbol_split)
recessive_candidates <- recessive_candidates %>%
  dplyr::select(where(~ !is.list(.)))

gene_map <- recessive_candidates %>%
  mutate(row_id = row_number()) %>%
  separate_rows(gene_symbol, sep = "\\|")

gene_scores <- gene_map %>%
  group_by(gene_symbol) %>%
  summarise(
    GeneScore_AvgAll = mean(VariantScore, na.rm = TRUE),
    GeneScore_AvgTop2 = mean(sort(VariantScore, decreasing = TRUE)[1:min(2, n())],
...
recessive_candidates <- gene_map %>%
  left_join(gene_scores, by = "gene_symbol") %>%
  group_by(row_id) %>%
  summarise(
    across(everything(), ~ dplyr::first(.[!is.na(.)])),
    .groups = "drop"
  ) %>%
  dplyr::select(-row_id)
...
find_enhanced_matches <- function(wb) {
  ...
  if ("gene_symbol" %in% colnames(sheet_i) && "gene_symbol" %in% colnames(sheet_j)) {
    ...
    genes_i <- unique(unlist(strsplit(sheet_i$gene_symbol, "\\|")))
    genes_j <- unique(unlist(strsplit(sheet_j$gene_symbol, "\\|")))
    
    return(base::intersect(genes_i, genes_j))
...
# Expand both sheets
expanded_i <- expand_genes(full_sheet_list[[i]])
expanded_j <- expand_genes(full_sheet_list[[j]])

# Find matching row IDs (deduplicated)
sheet_i_ids <- unique(expanded_i$row_id[expanded_i$gene_symbol %in% matches])
sheet_j_ids <- unique(expanded_j$row_id[expanded_j$gene_symbol %in% matches])

# Pull original rows
sheet_i_matches <- full_sheet_list[[i]][sheet_i_ids, ]
sheet_j_matches <- full_sheet_list[[j]][sheet_j_ids, ]

sheet_i_matches$SourceSheet <- sheet_names[[i]]
sheet_j_matches$SourceSheet <- sheet_names[[j]]

# Group if structure supports it
if (all(c("CHROM", "SV_Start", "SV_Stop") %in% names(sheet_i_matches))) {

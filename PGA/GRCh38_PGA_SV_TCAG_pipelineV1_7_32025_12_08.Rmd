---
title: "PGA_SV_hg38_07.25 - Panel-Gene SV analysis for hg38 builds. TCAG pipeline28_5"
...
Dprompt <- function() {
  Dpromt <- paste0("Please write the Sample ID:")
...
setup <- function(){
  ...
  "This script was built to filter SV & CNVnator files in parallel. Follow the prompt instructions."
)
...
MANTAFilter <- function(){
  ...
  currentdata <<- dplyr::filter(currentdata, otgMantaPctFreq_90pctRecOvlp %in% NA | otgMantaPctFreq_90pctRecOvlp <=1)
   ...
   MANTAogText <<- "SV Database (1000g) Filter <= 1%"
  ...
  if (("svMantaXPctFreq_90pctRecOvlp" %in% names(currentdata))) {
      ...
      MANTAText <<- "SV Database (Internal) Filter <= 1%"
...
colnames(RefSeq)[c(1:16)] <- c("bin", "name", "chrom", "strand", "txstart", "txEnd", "cdsStart", "cdsEnd", "ExonCount", "exonStarts", "exonEnds", "score", "gene_symbol", "cdsStartStat", "cdsEndStat", "ExonFrames")
...
variants_with_exon_overlaps_easy <<- overlap_results %>%
  dplyr::mutate(overlapping_exons = overlap_details$overlapping_exons)%>%
  dplyr::select(-"cdsStartStat", -"cdsEndStat",-"ExonFrames", -exonEnds_list, -exonStarts_list)
}
...
wb <<- createWorkbook() 
sheet_name_1 <<- as.character("Filtered_SV")
  sheet_name_2 <<- as.character("Genic_SV")
  sheet_name_panel <<- as.character("Panel_SV")
  ...
  final_file <- paste0(Dnumber, "_panel_filtered_SV.xlsx")
...
# Check if the final file name already exists 
  if (file.exists(final_file)) {
    counter <- 1   
    dup_file <- paste0(Dnumber, "_", counter, "_panel_filtered_SV.xlsx")     

---
title: "WGA_SV_hg38_07.25 - Whole Genome SV analysis for hg38 builds for TCAG pipeline 28_5"
author: "Layla Ahmed"
date: "2025-10-12"
...
#This script was built to filter SV & CNVnator files in parallel;
#If you opt to load in your SV files through File -> Import Dataset -> Base (readr), please name your files 'patientdata_SV' & 'patientdata_CNV' so the script can recognise them.
...
file_choice_SV <- function(){
  if ("patientdata_SV" %in% ls(envir = .GlobalEnv)) {
    print ("File named 'patientdata_SV' found.")
  } else {
    print ("'patientdata_SV' is missing, please select a file:")
    patientfile <<- file.choose()
    patientfile_ext <<-  tools::file_ext(patientfile)
    if(patientfile_ext == "csv"){
      patientdata_SV <<- read.csv(patientfile, stringsAsFactors = FALSE)
    } else if(patientfile_ext == "tsv"){
      patientdata_SV <<- read.delim(patientfile, sep = "\t", strip.white = TRUE, stringsAsFactors = FALSE)
      ...
      patientdata_SV <<- read_xlsx(patientfile, sheet = sheet_name)
    }
...
patientfile_cmd_SV <- function(){
  ...
  names(patientdata_SV) <- sub(Dnumber_toremove_colon, "", names(patientdata_SV))
  names(patientdata_SV) <- sub(Dnumber_toremove_period, "", names(patientdata_SV))
  dup_cols <- names(patientdata_SV)[duplicated(names(patientdata_SV))]
  # Append a suffix to duplicate column names to make them unique
  for (col in dup_cols) {
    indices <- which(names(patientdata_SV) == col)
    names(patientdata_SV)[indices] <- paste(col, indices, sep = "_")
  }
  InitialRow <- nrow(patientdata_SV)
  InitialText <- "Initial Number of SV Variants"
  currentdata_SV <<- patientdata_SV
  FilterStep_SV$Step <<- c(InitialText)
  FilterStep_SV$Count <<- c(InitialRow)
  }
}
...
SVFilter <- function(){
  ...
  if (("otgDgnPctFreq_90pctRecOvlp" %in% names(currentdata))) {
  currentdata <<- dplyr::filter(currentdata, otgDgnPctFreq_90pctRecOvlp %in% NA | otgDgnPctFreq_90pctRecOvlp <=1)
   SVogRow <<- nrow(currentdata)
   SVogText <<- "SV Database (1000g) Filter <= 1%"
   currentdata_SVog <<-currentdata
  
  } else if (("otgMantaPctFreq_90pctRecOvlp" %in% names(currentdata))) {
      currentdata_SVog <<- dplyr::filter(currentdata, otgMantaPctFreq_90pctRecOvlp %in% NA | otgMantaPctFreq_90pctRecOvlp <=1) 
      SVogRow <<-nrow(currentdata)
      SVogText <<- "SV Database (1000g) Filter <= 1%"
      currentdata_SVog <<-currentdata
  } else if (("otgDellyPctFreq_90pctRecOvlp" %in% names(currentdata))) { currentdata_SVog <<- dplyr::filter(currentdata,             otgDellyPctFreq_90pctRecOvlp %in% NA | otgDellyPctFreq_90pctRecOvlp <=1) 
      SVogRow <<-nrow(currentdata)
      SVogText <<- "SV Database (1000g) Filter <= 1%"
      currentdata_SVog <<-currentdata}
  else {
    SVogRow <<- as.character("Not Applicable")
    SVogText <<- "SV Database (1000g) Filter <= 1%"
    currentdata_SVog <<-currentdata
    
  }
  
  if (("eur_DgnPctFreq_90pctRecOvlp" %in% names(currentdata))) {
      currentdata <<- dplyr::filter(currentdata,eur_DgnPctFreq_90pctRecOvlp %in% NA |  eur_DgnPctFreq_90pctRecOvlp <=0.5) 
      SVRow <<- nrow(currentdata)
      SVText <<- "eur_DgnPctFreq_90pctRecOvlp <= 1%"
      currentdata_SV <<-currentdata

  } else if (("all_DgnPctFreq_90pctRecOvlp" %in% names(currentdata))) {
      currentdata <<- dplyr::filter(currentdata, all_DgnPctFreq_90pctRecOvlp %in% NA | all_DgnPctFreq_90pctRecOvlp <=1) 
      SVRow <<- nrow(currentdata)
    SVText <<- "SV Database (Internal) Filter <= 1%"
    currentdata_SV <<-currentdata
  } else {
    SVRow <<- as.character("Not Applicable")
    SVText <<- "SV Database (Internal) Filter <= 1%"
    currentdata_SV <<-currentdata
  }
 ...
 currentdata_DGV <<-currentdata
   ...
   if (("gnomAD_SF_POPMAX_max50pctRecOvlp" %in% names(currentdata))){
  currentdata <<-  dplyr::filter(currentdata, gnomAD_SF_POPMAX_max50pctRecOvlp %in% NA | gnomAD_SF_POPMAX_max50pctRecOvlp <=0.5)
  gADRow <<- nrow(currentdata)
  gADText <<- "gnomAD Database (All Studies) Filter <= 0.5%"
 currentdata_gAD <<-currentdata
  } else if (("gnomAD_AF_MAX_max90pctRecOvlp" %in% names(currentdata))){
  currentdata <<-  dplyr::filter(currentdata,gnomAD_AF_MAX_max90pctRecOvlp %in% NA | gnomAD_AF_MAX_max90pctRecOvlp <=0.5)
  gADRow <<- nrow(currentdata)
 gADText <<- "gnomAD Database (All Studies) Filter <= 0.5%"
 currentdata_gAD <<-currentdata
  } else {
    gADRow <<- as.character("Not Applicable")
 gADText <<- "gnomAD Database (All Studies) Filter <= 0.5%"
 currentdata_gAD <<-currentdata
    ...
    if (("Fantom5DPI" %in% names(currentdata))){
  Fantom5Data_SV <<-  dplyr::filter(currentdata, Fantom5DPI >= 2)
  Fantom5Row <<- nrow(Fantom5Data_SV)
  Fantom5RowText <<- "SV with Fantom5 = 2"
  }  else {
    Fantom5Data_SV <<- data.frame()
    Fantom5Row <<- as.character("Not Applicable")
 Fantom5RowText <<- "SV with Fantom5 = 2"

    
  }
   FilterStep_SV$Step <<- c(FilterStep_SV$Step, CleanText, SVogText, SVText, DellyText, DRGNText, DGVText, gADText,Fantom5RowText)
  FilterStep_SV$Count <<- c(FilterStep_SV$Count, CleanRow, SVogRow, SVRow, DellyRow, DRGNRow, DGVRow, gADRow, Fantom5Row)
...
CNVFilter <- function(){
 ...
 currentdata_gAD <<-currentdata
    ...
    if (("Fantom5DPI" %in% names(currentdata))){
  Fantom5Data_CNV <<-  dplyr::filter(currentdata, Fantom5DPI >= 2)
  Fantom5Row <<- nrow(Fantom5Data_CNV)
  Fantom5RowText <<- "CNV with Fantom5 = 2"
  }  else {
    Fantom5Data_CNV <<- data.frame()
    Fantom5Row <<- as.character("Not Applicable")
 Fantom5RowText <<- "CNV with Fantom5 = 2"
...
MANTA_Filtering <- function(){
  if ("currentdata_SV" %in% ls(envir = .GlobalEnv)) {
   SVFilter()
  what_column_chrom()
  what_column_gene()
  what_column_start()
...
if ("gene_symbol" %in% names(currentdata)){
  ...
  currentdata_genic <<- currentdata_genic %>%
    separate_rows(gene_symbol, sep = regex("\\|"))
...
FilterStep_SV$Step <<- c(FilterStep_SV$Step, GenicText, GenesInvovlvedText)
FilterStep_SV$Count <<- c(FilterStep_SV$Count,GenicRow, GenesInvovlvedRow) }
  else {
FilterStep_SV$Step <<- c(FilterStep_SV$Step, GenesInvovlvedText)
FilterStep_SV$Count <<- c(FilterStep_SV$Count, GenesInvovlvedRow)
  }

currentdata_genic_SV <<- currentdata_genic
...
#currentdata_genic_SV <<- currentdata_genic_SV[trimws(currentdata_genic_SV$gene_symbol) != "", ]
  #Genesm_row <<-  nrow(currentdata_genic_SV)
  #Genesm_text <<- "Genes Invovled in SVs"
  
  #currentdata_geneSV_SV <<- currentdata_genic_SV %>%
   #  group_by(CHROM, SV_Start, SV_Stop) %>%
    #summarise(
    #gene_symbol = paste(unique(gene_symbol), collapse = ","), # Combine gene_symbol
    #across(everything(), ~ dplyr::first(.)), # Keep the first value for other columns
    #.groups = 'drop'
  #)
  #Genesvm_row <<-  nrow(currentdata_geneSV_SV)
  #Genesvm_text <<- "Unique SVs (with Genes)"
   currentdata_Mfilter <<- currentdata_genic_SV
...
overlap_results_SV <<- overlap_results %>%
  group_by(gene_symbol) %>%  
  slice_max(ExonCount, with_ties = FALSE) %>%  # Keep only the highest ExonCount per gene_symbol
  ungroup()
variants_with_exon_overlaps_SV <<- variants_with_exon_overlaps %>%
  group_by(gene_symbol) %>%  
  slice_max(ExonCount, with_ties = FALSE) %>%  # Keep only the highest ExonCount per gene_symbol
  ungroup()
...
variants_with_exon_overlaps_easy_SV <<- variants_with_exon_overlaps_easy

CDSMA_row <<-  nrow(variants_with_exon_overlaps_easy_SV)
  CDSMA_text <<- "Unique SV w CDS Overlap"
  
  FilterStep_SV$Step <<- c(FilterStep_SV$Step,CDSMA_text)
  FilterStep_SV$Count <<- c(FilterStep_SV$Count, CDSMA_row) 
 ...
 FilterStep_SV <<- data.frame(
  Step = FilterStep_SV$Step,
  Count = FilterStep_SV$Count,
  stringsAsFactors = FALSE
)  
wb <<- createWorkbook() 
sheet_name_1 <<- as.character("Rare_SV")
sheet_name_2 <<- as.character("Genic_SV")
sheet_name_4 <<- as.character("Fantom5_Regulatory_SV")
sheet_name_5 <<- as.character("SVs Crossing CDS (Gene-Sep)")
sheet_name_6 <<-as.character("SVs Crossing CDS (SV)")
sheet_name_3 <<- as.character("SV_Filter_History")
  ...
  addWorksheet(wb, sheet_name_6)
    ...
    final_file <- paste0(Dnumber, "_filtered_SVs.xlsx")
  writeData(wb, sheet = sheet_name_1, x = currentdata_Mfilter, na.string = "NA")
  writeData(wb, sheet = sheet_name_2, x = currentdata_genic, na.string = "NA")
  writeData(wb, sheet = sheet_name_4, x = Fantom5Data_SV, na.string = "NA")
  writeData(wb, sheet = sheet_name_5, x = variants_with_exon_overlaps_SV, na.string = "NA")
  writeData(wb, sheet = sheet_name_6, x = variants_with_exon_overlaps_easy_SV, na.string = "NA")
  writeData(wb, sheet = sheet_name_3, x = FilterStep_SV, na.string = "NA")
# Check if the final file name already exists 
  if (file.exists(final_file)) {
    counter <- 1   
    dup_file <- paste0(Dnumber, "_filtered_SVs", counter, ".xlsx")     
...
wb <<- createWorkbook()
sheet_name_1 <<- as.character("Rare_CNV")
  sheet_name_2 <<- as.character("Genic_CNV")
  sheet_name_3 <<- as.character("CNV_Filter_History")
  sheet_name_4 <<- as.character("Fantom5_Regulatory_CNV")
  ...
  writeData(wb, sheet = sheet_name_4, x = Fantom5Data_CNV, na.string = "NA")
...
filter_choice <- function(){
  ...
  printline <- cat("What will you be filtering today? \n[1] MANTA (Also called SV-DRAGEN) \n [2] CNVnator \n [3] Both MANTA/SV-DRAGEN & CNVnator")
